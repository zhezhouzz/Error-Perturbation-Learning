...
type 'a t = T | F | Leaf of 'a | Node of 'a * 'a t * 'a t
...
let eval (dt : feature t) m : bool =
  let rec aux = function
    ...
    | Node (feature, l, r) -> if F.eval feature m then aux l else aux r
  in
  aux dt
...
let eval_vector (dt : feature t) (fl : feature list) (vec : bool list) : bool =
  ...
  let rec aux = function
    ...
    | Node (feature, l, r) -> if get_b feature then aux l else aux r
  in
  aux dt

let eval_vector_idx (dt : int t) (vec : bool array) : bool =
  let nth i = try vec.(i) with _ -> raise @@ failwith "eval_vector_idx" in
  let rec aux = function
    | T -> true
    | F -> false
    | Leaf feature -> nth feature
    | Node (feature, l, r) -> if nth feature then aux l else aux r
  in
  aux dt
...
let to_forallformula (qv : T.tvar list) (dtree : feature t) = (qv, to_prop dtree)
...
let to_spec (args : T.tvar list) (qv : T.tvar list) (dtree : feature t) =
  Specification.Spec.{ args; qv; body = to_prop dtree }
...
let dt_summary dt fset =
  ...
  let rec next idx =
    ...
    else if not fv_arr.(idx) then (
      Array.set fv_arr idx true;
      Some idx)
    else
  ...
  let rec aux idx =
    ...
    let _ =
      Printf.printf "iter:%s\n" (List.split_by_comma string_of_bool fvec)
    in
    let _ =
      if eval_vector dt fset fvec then posnum := !posnum + 1
...
(*   in *)
(*   aux 0; *)
(*   classify_hash fset ftab is_pos *)
...
(* let label_encode = function Pos -> `Int 0 | MayNeg -> `Int 1 | Neg -> `Int 2 *)
...
let count size dt =
  ...
  let rec aux prefix = function
    ...
    | Node (x, l, r) ->
        if List.exists (fun y -> y == x) prefix then
