...
let spec_infer ctx (data: 'a list) (to_values: 'a -> V.t list) (judge: 'a -> bool) =
  ...
  let () = Zlog.log_write @@ spf "pos:\n%s\n" (List.split_by "\n" (fun x -> x) @@ List.map V.layout_l pos_values) in
  let () = Zlog.log_write @@ spf "neg:\n%s\n" (List.split_by "\n" (fun x -> x) @@ List.map V.layout_l neg_values) in
  if List.length neg_values == 0
  then Specification.Spec.({args = ctx.Cctx.args; qv = ctx.Cctx.qv; body = Specification.Specast.True})
  else if List.length pos_values == 0
  then
    (Zlog.log_write ~log_level:LWarning "the precondition is false, prob is wrong";
     Specification.Spec.({args = ctx.Cctx.args; qv = ctx.Cctx.qv; body = Specification.Specast.(Not True)}))
  else
    ...
    Specification.Spec.({args = ctx.Cctx.args; qv = ctx.Cctx.qv; body = body})
