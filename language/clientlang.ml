...
let layout_body =
  let rec aux = function
    ...
    | Match (a, cases) ->
      List.fold_left (fun str (cobody, body) ->
          spf "%s| %s ->\n%s\n" str (aux cobody) (aux body)
        ) (spf "match %s with\n" @@ List.split_by_comma (fun x -> x) a) cases
  in
  aux

let layout_env env = Printf.sprintf "{%s}" @@
  List.split_by_comma (fun (name, value) -> Printf.sprintf "%s->%s" name @@ V.layout_l value) @@ StrMap.to_kv_list env
...
let rev_eval libmember libinsp term values env =
  ...
  let rec aux term values =
    ...
    | Tuple vars -> merge @@ List.map (fun (arg, value) -> aux arg [value]) @@ List.combine vars values
    | App (fname, args) ->
      (match Bblib.invocation_inspector_rev_call libinsp fname values with
      | None -> None
      | Some values' ->
        if List.length values' != List.length args
        then raise @@ failwith "Fatal Error: type error which should be handled"
        else
          Sugar.(let* l, trace = aux (Tuple args) values' in
                 Some (l, fname :: trace)
                )
      )
  ...
  | Some (l, trace) ->
    ...
    Some (
      List.fold_left (fun env (name, values) ->
        match StrMap.find_opt env name with
        | Some _ -> raise @@ failwith "Sementic Error: match case should not rename existing variables"
...
let eval funcdef libinsp inputs =
  let lib_members = StrMap.from_kv_list @@
    List.map (fun (_, name, v) -> name, v) @@ Bblib.lib_get_member libinsp in
  let mkenv inputs =
  ...
  let rec aux (env: V.t list StrMap.t) t : V.t list option =
      ...
      (match StrMap.find_opt env var with
       | None ->
         (match StrMap.find_opt lib_members var with
          ...
          | None -> Zlog.log_write (spf "client eval: cannot find %s" var); None)
    ...
    | Let (lfs, rhs, body) ->
      ...
      let env' = StrMap.add_seq (List.to_seq @@ List.map (fun (a,b) -> a, [b]) @@ List.combine lfsnames values) env in
      aux env' body
    | Match (terms, cases) ->
