...
type 'a type_check_monad = TySafe of 'a | TyErr of string

let tc_bind (tcr : 'a type_check_monad) (f : 'a -> 'b type_check_monad) =
  match tcr with TyErr msg -> TyErr msg | TySafe a -> f a
...
let tpenv_safe_combine_one (old : Tp.t list StrMap.t) name tp =
  ...
  | Some tp' ->
      if Tp.tps_eq tp tp' then TySafe old
      else
        TyErr
          (spf "type of %s cannot be unified: %s vs. %s" name (Tp.layout_l tp)
             (Tp.layout_l tp'))

let tpenv_safe_diff (old : Tp.t list StrMap.t) (newone : Tp.t list StrMap.t) =
  StrMap.fold
    (fun name tp (result : unit type_check_monad) ->
      match StrMap.find_opt old name with
      | None -> result
      | Some tp' -> (
          if Tp.tps_eq tp tp' then TySafe ()
          else
            let msg =
              spf "type of \"%s\" cannot be unified: %s vs. %s" name
                (Tp.layout_l tp) (Tp.layout_l tp')
            in
            match result with
            | TySafe _ -> TyErr msg
...
let type_check libdef funcdef =
  ...
  let handle_function inpt outt args expecteds =
    ...
    if List.length inpt != List.length args then
      TyErr
        (spf
           "the number of args(%s) does not match the function requirement(%s)"
           (List.split_by_comma Clientlang.layout_body args)
           (List.split_by_comma Tp.layout inpt))
    else TySafe (Tuple args, inpt)
  in
  let rec aux tenv t (expecteds : T.t list) =
    ...
    | Op (op, args) ->
        let* inpt, outt =
          match Clientlang_op.tp op with
          | Some (inpt, outt) -> TySafe (inpt, outt)
          | None -> TyErr (spf "cannot find type of operator %s" op)
        in
    ...
    | App (fanme, args) when String.equal funcdef.fname fanme ->
        ...
        let* t', expecteds' = handle_function inpt outt args expecteds in
        aux tenv t' expecteds'
    | App ("raise", _) -> TySafe tenv
    | App (fname, args) ->
        let* inpt, outt =
          match Bblib.lib_get_tp_by_name libdef fname with
          | None ->
              Hashtbl.iter
                (fun name _ -> Printf.printf "knwon lib: %s\n" name)
                libdef.m;
              TyErr
                (spf "cannot find the definition of library function %s" fname)
          | Some x -> TySafe x
        in
        let* t', expecteds' = handle_function inpt outt args expecteds in
        aux tenv t' expecteds'
    ...
    | Let (lfs, rhs, body) ->
        ...
        aux
          (StrMap.add_seq (List.to_seq @@ kv_list_from_args lfs) tenv)
          body expecteds
    | Match (terms, cases) ->
        ...
        let* terms_tp =
          List.fold_left
            (fun r name ->
              ...
              | None ->
                  TyErr
                    (spf "match here need the type of the matched term %s" name)
              | Some tp -> TySafe (r @ tp))
            (TySafe []) terms
        in
        List.fold_left
  ...
  and aux_multi tenv ts expecteds =
    if List.length ts != List.length expecteds then
      TyErr
        (spf "the number of variables(%s) does not match the types(%s)"
      ...
      List.fold_left
        (fun tenv (t, expected) ->
          ...
          aux tenv t [ expected ])
        (TySafe tenv)
      @@ List.combine ts expecteds
  in
  let lib_members =
    List.map (fun (tp, name, _) -> (name, [ tp ]))
    @@ Bblib.lib_get_member libdef
  in
  let tenv =
    StrMap.from_kv_list @@ lib_members @ kv_list_from_args funcdef.args
  in
