...
let type_reduction tpname =
  let tpname' = try Bblib.underline_type_reduction tpname with
    | _ -> tpname in
  Tp.of_string tpname'
...
let expr_of_ocamlexpr expr =
  let rec aux expr =
    ...
    | Pexp_apply (func, args) ->
      let funcname = match aux func with
        | L.Var x -> x
...
let prop_of_ocamlexpr tenv expr =
  let rec aux expr =
    ...
    | Pexp_apply (func, args) ->
      let funcname = match expr_of_ocamlexpr func with
        | L.Var x -> x
        | _ -> raise @@ failwith "Can only allow function name as function"
      in
      ...
      (match funcname, args with
       ...
       | _ ->
         let targs = List.map (fun e ->
             let name = match expr_of_ocamlexpr e with
               | L.Var x -> x
               | _ -> raise @@ failwith "Method predicate does not allow nested predicates" in
...
(* Client Code *)
type client = {clientname: string; clientargs: T.tvar list; clientbody: Tinyocaml.t}
let client_of_ocamlstruct structure =
  ...
  | Pstr_value (_, [value_binding]) ->
    ...
    let clientargs, clientbody = args_body_of_ocamlexpr value_binding.pvb_expr in
...
(* Assertion *)
type assertion = {assertionname: string; assertionargs: T.tvar list; assertionbody: SpecAst.t}
let assertion_of_ocamlstruct structure =
  match structure.pstr_desc with
  | Pstr_value (_, [value_binding]) ->
    let assertionname = pattern_to_string value_binding.pvb_pat in
    let assertionargs, assertionbody = args_body_of_ocamlexpr value_binding.pvb_expr in
    let tenv = List.fold_left (fun tenv (tp, name) ->
        StrMap.add name tp tenv
    ) StrMap.empty assertionargs in
    let assertionbody = prop_of_ocamlexpr tenv assertionbody in
    {assertionname; assertionargs; assertionbody}
  | _ -> raise @@ failwith "translate not a function value"

(* Meta *)
let get_p asts idx = try List.nth asts idx with
  | _ -> raise @@ failwith (spf "missing %ith structure" idx)
let get_meta name asts idx =
  let p = get_p asts idx in
  let name', expr = match p.pstr_desc with
    | Pstr_value (_, [value_binding]) ->
      pattern_to_string value_binding.pvb_pat, value_binding.pvb_expr
    | _ -> raise @@ failwith "parse_meta: wrong format"
  in
  if not (String.equal name name')
  then raise @@ failwith (spf "parse_meta: %s is expected" name)
  else expr
...
let load_meta asts =
  let get_strings expr =
    (match expr.pexp_desc with
     ...
     | _ -> raise @@ failwith "parse_assertion::strings")
  in
  let get_int expr = match expr_of_ocamlexpr expr with
    | Lit [V.I i] -> i
    | _ -> raise @@ failwith "parse_meta: int value is expected"
  in
  let get_lit expr = match expr_of_ocamlexpr expr with
    | Lit lit -> lit
    | _ -> raise @@ failwith "parse_meta: int value is expected"
  in
  ...
  | "pre" ->
    ...
    preds, op_pool, libs, i_err, sampling_rounds, p_size, assertion1, assertion2
  | "post" ->
    preds, op_pool, libs, i_err, sampling_rounds, p_size,
    {assertionname = "pre"; assertionargs = []; assertionbody = SpecAst.True} , assertion1
  | _ -> raise @@ failwith "parse_meta: pre/post are expected"

let slipt_args argtps assertionargs =
  if List.length assertionargs < List.length argtps
  then raise @@ failwith "the length of args of client does not match the args of the assertions"
  else
    let args' = List.sublist assertionargs (0, List.length argtps) in
    let rest = List.sublist assertionargs (List.length argtps, List.length assertionargs) in
...
let post_to_spec args rettps {assertionargs; assertionbody; _} =
  ...
  | SpecAst.True -> (* When the preconditino is omitted *)
...
let load_client_and_meta source_client source_assertions =
  ...
  let rettps = List.map type_reduction @@ load_rettps @@ get_p source_client 0 in
  let {clientname; clientargs; clientbody} = client_of_ocamlstruct @@ get_p source_client 1 in
  let preds, op_pool, libs, i_err, sampling_rounds, p_size, sigma, phi = load_meta source_assertions in
  ...
  let client_func = L.({fname = clientname; args = clientargs; body = clientbody; res = rettps}) in
