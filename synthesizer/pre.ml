...
let perturbation_pre_infer (cctx: Cctx.t) (acache: Sampling.cache) (sigma: V.t list -> bool)
  ...
  | _ ->
    ...
    in
    Infer.spec_infer cctx io_list to_value to_label

let max_qv_num = 100
let qv_name_space = ["u"; "v"; "w";] @ List.init max_qv_num (fun i -> Printf.sprintf "k%i" i)

let pre_infer_from_env env qvnum =
  if qvnum > max_qv_num then raise @@ failwith (spf "the max qv num is %i\n" max_qv_num) else
    let qv = List.init qvnum (fun i -> Tp.Int, List.nth qv_name_space i) in
    let open Env in
    match env.cur_p with
    | None -> raise @@ failwith "Precondition Inference: the perturbation function does not exists."
    | Some cur_p ->
      let args = List.map (fun (tp, idx) -> tp, Language.Oplang.layout_var (tp, idx)) cur_p.prog.fin in
      let cctx = Cctx.mk_cctx args qv env.preds in
      let () = Printf.printf "fset: %s\n" (Feature.layout_set cctx.Cctx.fset) in
      let scache = Sampling.cost_sampling_ env.Env.tps [env.Env.i_err] cur_p.prog env.sampling_rounds in
      perturbation_pre_infer cctx scache
        env.sigma
