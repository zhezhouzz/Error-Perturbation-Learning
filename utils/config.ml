...
type mutation_distribution = {op_replace: int;
                              op_swap: int;
                              op_deny: int;
                              arg_reassign: int;}
type config = {exec_flag: exec_flag; if_random: bool; z3_ctx: Z3.context option; mutation_distribution: mutation_distribution}
let conf = ref {if_random = true; exec_flag = Opt; z3_ctx = None;
                mutation_distribution = {op_replace = 1;
                                         op_swap = 1;
                                         op_deny = 1;
                                         arg_reassign = 1;}
               }
...
let load_config configfile =
  ...
  let if_random =
    try j |> member "if_random" |> to_bool with
    | _ -> raise @@ failwith "cannot load config::if_random" in
  let if_z3 =
    try j |> member "if_z3" |> to_bool with
  ...
  let mutation_distribution =
    try
      ...
      {op_replace = md |> member "op_replace" |> to_int;
       ...
       arg_reassign = md |> member "arg_reassign" |> to_int;} with
    | _ -> raise @@ failwith "cannot load config::mutation_distribution" in
  conf := {exec_flag = exec_flag; if_random = if_random; z3_ctx = z3_ctx; mutation_distribution = mutation_distribution}
