...
let subst update e =
  let rec aux e =
    ...
    | True -> e
    | Bvar (tp, b) ->
        let tp', b' = update (tp, b) in
        Bvar (tp', b')
    | MethodPredicate (mp, args) -> MethodPredicate (mp, List.map update args)
    | Implies (e1, e2) -> Implies (aux e1, aux e2)
    | Ite (e1, e2, e3) -> Ite (aux e1, aux e2, aux e3)
    | Not e -> Not (aux e)
    | And l -> And (List.map aux l)
...
let rec layout = function
  ...
  | Iff (p1, p2) -> sprintf "(%s %s %s)" sym_iff (layout p1) (layout p2)
  | Ite (p1, p2, p3) ->
...
let pretty_layout indent e =
  ...
  let rec aux indent = function
    ...
    | MethodPredicate ("<", [ a; b ]) -> sprintf "(%s < %s)" (snd a) (snd b)
    | MethodPredicate (mp, args) ->
        sprintf "%s(%s)" (MP.poly_name mp) (List.split_by_comma snd args)
    | Implies (p1, p2) ->
        if is_display_level_lit p2 then
          sprintf "%s(%s %s %s)" (mk_indent indent) (aux 0 p1) sym_implies
            (aux 0 p2)
        else
    ...
    | And ps ->
        if List.for_all is_display_level_lit ps then
          sprintf "%s(%s)" (mk_indent indent)
          @@ List.split_by (sprintf " %s " sym_and) (aux 0) ps
        else
          sprintf "%s(\n%s\n%s)" (mk_indent indent)
            (List.inner_layout
    ...
    | Or ps ->
        if List.for_all is_display_level_lit ps then
          sprintf "%s(%s)" (mk_indent indent)
          @@ List.split_by (sprintf " %s " sym_or) (aux 0) ps
        else
          sprintf "%s(\n%s\n%s)" (mk_indent indent)
            (List.inner_layout
               (List.map (aux (indent + 1)) ps)
               ...
               "true")
            (mk_indent indent)
    | Not (MethodPredicate ("==", [ a; b ])) ->
        sprintf "(%s != %s)" (snd a) (snd b)
    | Not (MethodPredicate ("<", [ a; b ])) ->
        sprintf "(%s > %s)" (snd a) (snd b)
