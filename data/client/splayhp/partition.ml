  ...
  | _ when Splayhp.node x a b -> (
      if x <= pivot then
        ...
        | _ when Splayhp.node y b1 b2 ->
            if y <= pivot then
              let (small : Splayhp.t), (big : Splayhp.t) = partition y b2 in
              (Splayhp.node y (Splayhp.node x a b1) small, big)
            else
              let (small : Splayhp.t), (big : Splayhp.t) = partition y b1 in
        ...
        | _ when Splayhp.node y a1 a2 ->
            if y <= pivot then
              let (small : Splayhp.t), (big : Splayhp.t) = partition y a2 in
              (Splayhp.node y a1 small, Splayhp.node x big b)
            else
              let (small : Splayhp.t), (big : Splayhp.t) = partition y a1 in
